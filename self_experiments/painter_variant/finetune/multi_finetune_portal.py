...
from self_experiments.painter_variant.finetune.engine_train import train_one_epoch
...
def prepare_model(args, prints=False):
    ...
    if args.finetune:
        ...
        def freeze_match(name, f_list):
            ...
            return ret
        if args.load_frozen_only:
            for name in list(checkpoint.keys()):
                if not freeze_match(name, freeze_list): del checkpoint[name]
...
if __name__ == '__main__':
    ...
    args.load_frozen_only = False
    args.save_itrs = 5000
    args.mask_ratio = 0.5
    
    args.num_prompts = 2 # 2
    args.cr_depth = 9 # 16
    args.xcr_depth = 12 # 18
