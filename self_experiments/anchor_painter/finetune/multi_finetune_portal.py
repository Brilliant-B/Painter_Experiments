...
import models.anchor_painter.anchor_painter as models_painter

from self_experiments.anchor_painter.finetune.engine_train import train_one_epoch, evaluate_pt
...
def get_args_parser():
    ...
    if known_args.enable_deepspeed:
        try:
            ...
            eff_batch_size = known_args.batch_size * known_args.accum_iter * known_args.n_gpu
            if known_args.lr is None:  # only base_lr is specified
                parser.set_defaults(lr = known_args.blr * eff_batch_size / 256)
...
def main(args, ds_init):
    ...
    if True:  # args.distributed:
        ...
        if global_rank == 0:
            ...
            dataset_train.print_datasets()
            print("【val_datasets】")
            dataset_val.print_datasets()
            # print(dataset_train)
            # print(dataset_val)
    ...
    if global_rank == 0:
        print("initial ckpt: %s" % (args.finetune))
        ...
        with open(os.path.join(args.output_dir, "train_log.log"), 'a') as f:
            print("initial ckpt: %s" % (args.finetune), file=f)
            print("num epochs: %d" % (args.epochs), file=f)
            print("base lr: %.2e" % (args.lr * 256 / eff_batch_size), file=f)
            print("actual lr: %.2e" % args.lr, file=f)
            print("accumulate grad iterations: %d" % args.accum_iter, file=f)
            print("effective batch size: %d" % eff_batch_size, file=f)
            print("datasets:", file=f)
            for k, v in dataset_train.pair_type_dict.items():
                print(f"{k} ==> {len(v)}", file=f)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if global_rank == 0:
            print(f"Val loss of the network on the {len(dataset_val)} test images: Val Loss = {test_stats['loss']:.3f}")
            with open(os.path.join(args.output_dir, "train_log.log"), 'a') as f:
                print(f"Val loss of the network on the {len(dataset_val)} test images: Val Loss = {test_stats['loss']:.3f}", file=f)
    ...
    if global_rank == 0:
        ...
        with open(os.path.join(args.output_dir, "train_log.log"), 'a') as f:
            print('Training time {}'.format(total_time_str), file=f)
