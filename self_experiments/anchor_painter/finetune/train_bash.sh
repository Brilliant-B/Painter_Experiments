...
python -m torch.distributed.launch --nproc_per_node=${N_GPUS} --master_port=12358 \
	--use_env self_experiments/anchor_painter/finetune/multi_finetune_portal.py  \
    --n_gpu ${N_GPUS} \
    --batch_size 4 \
    --accum_iter 128 \
    --model anchor_painter_patch16_input896x448_win_dec64_8glb_sl1 \
    ...
    --output_dir workbench/train_anchor_painter \
    --log_dir workbench/train_anchor_painter/logs \
    --finetune pretrained/pretrained_mae_vit_large/mae_pretrain_vit_large.pth \
    
    # pretrained/pretrained_mae_vit_large/mae_pretrain_vit_large.pth \
