...
def train_one_epoch(model: torch.nn.Module,
    ...
    wandb_images = []
    for data_iter_step, (samples, targets, bool_masked_pos, valid) in enumerate(metric_logger.log_every(data_loader, print_freq, header, global_rank)):

        # we use a per iteration (instead of per epoch) lr scheduler
        if data_iter_step % accum_iter == 0:
            lr_sched.adjust_learning_rate_ori(optimizer, data_iter_step / len(data_loader) + epoch, args)
    ...
    print(f"[GPU{global_rank}] Averaged stats: {metric_logger}")
