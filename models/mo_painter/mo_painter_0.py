...
class Painter_Varient(nn.Module):
    ...
    def __init__(
        ...
        assert 0 <= self.mo <= 1, "momentum weight should be suitable"
        assert self.nc % self.nci == 0, "queue-length for simplicity concern"
    ...
    def context_queue_sampling(self, type, B, C):
        c, c_latent = [], []
        for i in range(B):
            ic = self.queues[type[i]].detach().cuda()
    ...
    def forward_encoder(self, type, c_query, c_target, query, target, mask):
        ...
        if not self.is_infer or (self.use_cache and self.cache is None):
...
def mo_painter_0_patch16_win_dec64_8glb_sl1(**kwargs):
