...
class Block(nn.Module):
    ...
    def forward(self, x, reg=None):
        # x: [B, ..., H, W, C]; (opt)reg: [B, ..., n, C]
        need_x_transform, need_reg_transform = not x.dim == 4, reg is not None and not reg.dim == 3
        if need_x_transform:
        ...
        if need_reg_transform:
            ...
            ret = (x, reg)
        else:   ret = x if reg is None else (x, reg)
        return ret



class T0_Painter(nn.Module):
    def __init__(
            self,
            img_size=(448, 448),
            patch_size=16,
        ...
        for i in range(self.collect_depth):
            c_block = Block(
                ...
                use_cait=not i<self.extract_layer,
        ...
        for i in range(self.depth):
            q_block = Block(
                ...
                window_size=int(i in window_block_indexes)*window_size,
                use_residual_block=i in residual_block_indexes,
                use_cait=not i<self.collect_depth,
                ls_init=init_layerscale,
                input_size=self.ori_window_size,
            )
            if use_act_checkpoint:
    ...
    def forward_encoder(self, type, c_query, c_target, query, target, mask):
        ...
        x = torch.stack([x, t], dim=1)[:, :, None, :, :, :] # (B, 2, 1, Hp, Wp, C)
        if self.pos_embed is not None:
        ...
        reg = self.registers.expand(B, self.nci, -1, -1) # reg: [B, nci, n, C]
        
        latents = []
        for idx in range(self.depth):
            if idx < self.collect_depth:
                x = self.blocks[idx](x)
                c = self.c_blocks[idx](c)
            elif idx < self.collect_layer:
                x = self.blocks[idx](x)
                c, reg = self.c_blocks[idx](c, reg)
            elif idx == self.collect_layer:
                reg = reg.flatten(1, 2) # reg: [B, nci*n, C]
...
def T0_patch16_win_dec64_8glb_sl1(**kwargs):
    model = T0_Painter(
