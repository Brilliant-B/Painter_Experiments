...
class T_Painter(nn.Module):
    def __init__(
        ...
        self.reg_embed = nn.Linear(embed_dim, embed_dim)
    ...
    @torch.jit.ignore
    def no_weight_decay(self):
        return {'pos_embed', 'reg_pos_embed', 'cls_token', 'registers'}
    ...
    def forward_loss(self, type, pred, target, mask, valid):
        ...
        inds_ign = ((target * imagenet_std + imagenet_mean) * (1 - 1. * image_mask)).sum((1, 2, 3)) < 50 * 3 # 100 * 3
        # if inds_ign.sum() > 0:
        #     valid[inds_ign] = 0.
...
def T2_patch16_win_dec64_8glb_sl1(**kwargs):
    model = T_Painter(
